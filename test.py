...
parser.add_argument(
    "--folder",
    default="dreampose-1",
    help="Path to custom pretrained checkpoints folder.",
)
parser.add_argument(
    "--pose_folder",
    default="../UBC_Fashion_Dataset/valid/91iZ9x8NI0S.mp4",
    help="Path to test frames, poses, and joints.",
)
parser.add_argument(
    "--test_poses",
    default=None,
...
parser.add_argument(
    ...
    default=44,
    required=True,
    help="Pretrained custom model checkpoint epoch number.",
)
parser.add_argument(
    "--key_frame_path",
    default="../UBC_Fashion_Dataset/dreampose/91iZ9x8NI0S.mp4/key_frame.png",
    help="Path to key frame.",
)
parser.add_argument(
    "--pose_path",
    default="../UBC_Fashion_Dataset/valid/A1F1j+kNaDS.mp4/85_to_95_to_116/skeleton_i.npy",
    help="Pretrained model checkpoint step number.",
)
parser.add_argument(
    "--strength",
    type=float,
    default=1.0,
    required=False,
    help="How much noise to add to input image.",
)
parser.add_argument(
    "--s1",
    type=float,
    default=0.5,
    required=False,
    help="Classifier free guidance of input image.",
)
parser.add_argument(
    "--s2",
    type=float,
    default=0.5,
    required=False,
    help="Classifier free guidance of input pose.",
)
parser.add_argument(
    "--iters",
    default=1,
    type=int,
    help="# times to do stochastic sampling for all frames.",
)
parser.add_argument("--sampler", default="PNDM", help="PNDM or DDIM.")
parser.add_argument(
    "--n_steps", default=100, type=int, help="Number of denoising steps."
)
...
parser.add_argument(
    ...
    default=0,
    required=False,
    help="Lowest predicted frame id.",
)
parser.add_argument(
    "--max_j",
    type=int,
    default=-1,
    required=False,
    help="Max predicted frame id.",
)
parser.add_argument(
    "--custom_vae", default=None, help="Path use custom VAE checkpoint."
)
parser.add_argument(
    "--batch_size",
    type=int,
...
save_folder = (
    args.output_dir if args.output_dir is not None else args.folder
)  #'results-fashion/'
...
frame_numbers = sorted(
    [
        int(path.split("frame_")[-1].replace("_densepose.npy", ""))
        for path in glob.glob(f"{args.pose_folder}/frame_*.npy")
    ]
)
